...
class Ui_newChanDialog(object):
    ...
    def retranslateUi(self, newChanDialog):
        ...
        self.label.setText(_translate("newChanDialog", "<html><head/><body><p>The simplest way to get into a new chan is to activate 'Create a new chan' above and enter the chan name. 'Join a chan' is unecessarily complicated employing a BM- address.    ----        A chan exists when a group of people share the same decryption keys. The keys and bitmessage address used by a chan are generated from a human-friendly word or phrase (the chan name). To send a message to everyone in the chan, send a normal person-to-person message to the chan address.</p><p>Chans are experimental and completely unmoderatable.</p></body></html>", None))
